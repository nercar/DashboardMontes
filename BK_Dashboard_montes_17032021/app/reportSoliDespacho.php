...
$sql = "SELECT RIGHT((CAST('00000' AS VARCHAR) + CAST(solipedi_id AS VARCHAR)), 5) AS pedido
		FROM BDES.dbo.BISolicPedido WHERE solipedi_nrodespacho = '$nropedido'";
$sql = $db->query($sql);
$pedidos = '';
...
$sql2 = "SELECT
			...
			(SELECT TOP 1 descripcion
				FROM BDES.dbo.ESProveedores WHERE codigo = 
				(SELECT TOP 1 TERCERO FROM BDES.dbo.BIKARDEX_DET
				WHERE TIPO = 1 AND MATERIAL = d.solipedidet_codigo
				GROUP BY TERCERO, FECHA	ORDER BY FECHA DESC)
			) AS proveedor,
			( SELECT TOP 1 a.descripcion FROM BDES.dbo.ESARTICULOS AS a WHERE a.codigo= d.solipedidet_codigo ) AS descripcion,
			( SELECT TOP 1 barra FROM BDES.dbo.ESCodigos AS b WHERE b.escodigo = d.solipedidet_codigo ORDER BY barra DESC ) AS barra,
