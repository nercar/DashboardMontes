		...
		switch ($opcion) {
			...
			case 'exportDetconSaldoBonos':
				$sql = "SELECT se.id_empresa, b.nom_empresa, b.dir_empresa, b.tel_empresa,
						...
						INNER JOIN
							(SELECT DISTINCT id_empresa, nom_empresa, dir_empresa, tel_empresa
							FROM BDES_POS.dbo.DBBonos) AS b ON b.id_empresa = se.id_empresa
						INNER JOIN BDES_POS.dbo.ESCLIENTESPOS AS cl ON cl.RIF = se.id_beneficiario";
			...
			case 'calendario_vtas':
				...
				$sql = "SELECT (CASE WHEN SUM(subtotal) = 0 THEN 0 ELSE ((SUM(subtotal) - SUM(costo)) / SUM(subtotal)) * 100 END) AS margen
						FROM total_global
						WHERE fecha = '";
				$sql.= date('Y-m-d', strtotime($fecha[0]."- 1 days")) . "'";
				...
				// Se prepara la consulta con los parametros
				$sql = "SELECT tg.fecha, SUM(tg.transacciones) AS transacciones,
						SUM(tg.cantidad) AS cantidad, SUM(tg.subtotal) AS subtotal,
						SUM(tg.costo) AS costo, COALESCE(td.tasa, 0) AS tasa
						FROM total_global tg
						LEFT OUTER JOIN tasas_dolar td ON td.fecha = tg.fecha
						WHERE tg.fecha BETWEEN '$fecha[0]' AND '$fecha[1]'";

				if($idpara!='localidad') {
					$sql .= " AND localidad = '$idpara'";
				}

				$sql .= " GROUP BY tg.fecha, td.tasa ORDER BY tg.fecha ASC";
				...
				for($i=0; $i<count($datos); $i++) {
					for($j=0; $j<count($row); $j++) {
						if($datos[$i]['fecha']==$row[$j]['fecha']) {
							...
							$datos[$i]['tasa']     = $row[$j]['tasa'];
							$datos[$i]['subtotal'] = $row[$j]['subtotal'];
							if($row[$j]['subtotal']==0) {
								$margen = 0.00;
							} else {
								round($margen = (($row[$j]['subtotal'] - $row[$j]['costo']) / $row[$j]['subtotal']) * 100, 2);
							}
							$datos[$i]['margen']   = number_format($margen, 2);
							switch (true) {
								case $margen > $margen_ant:
									$nivel = 'badge badge-success';
									break;
								
								case $margen < $margen_ant:
									$nivel = 'badge badge-danger';
									break;
								default:
									$nivel = 'badge badge-secondary';
									break;
							}
							$datos[$i]['nivel']   = $nivel;
			...
			case 'calendario_vtas_hoy':
				...
				$sql = "SELECT MAX(fecha) AS fecha,
							COUNT(DISTINCT CONCAT(localidad, '-', factura, '-', caja)) AS transacciones,
							SUM(cantidad) AS cantidad, SUM(subtotal) AS subtotal, SUM(costo) AS costo,
							COALESCE((SELECT tasa FROM tasas_dolar WHERE fecha = CURRENT_DATE), 0) AS tasa
						FROM detalle
				...
				if($row['subtotal']==0) {
					...
					round($margen = (($row['subtotal'] - $row['costo']) / $row['subtotal']) * 100, 2);
				}
				$datos['margen']   = number_format($margen, 2);
				switch (true) {
					case $margen > $margen_ant:
						$nivel = 'badge badge-success';
						break;
			...
			case 'guardarTranpTransf':
				...
				$sql  = "INSERT INTO BDES.dbo.DBCabtransptransf(fecha, codtransp, codcond, prescinto, chof_particular, tran_particular)
						 VALUES (CURRENT_TIMESTAMP, $codtransp, $codcond, '$precinto', '$chopar', '$trapar');";

				if($connec->query($sql)) {
					$sql = "SELECT IDENT_CURRENT('BDES.dbo.DBCabtransptransf') AS lastId;";

					$sql = $connec->query($sql);
					$datos = $sql->fetch();
					$id_act = $datos['lastId'];

					$sql = "INSERT INTO BDES.dbo.DBDettransptransf(id, documento, tipo, locori, tipoproc, eliminado, locdes) VALUES ";
					for ($i=0; $i < count($idpara); $i++) { 
						$sql .= "($id_act, " . $idpara[$i] . ", 17, 99, 1, 0, '$locdes'),";
					}
					$sql = substr($sql, 0, -1);
					$sql = $connec->query($sql);
					if($sql) {
						$idpara = implode(',', $idpara);
						$sql = "UPDATE BDES.dbo.BIKARDEX WITH (READPAST) SET
									OBSERVACION = '1'
								...
								WHERE DOCUMENTO_SERIE IN($idpara)
									...
									AND CODIGO = 358;";  // 358 codigo de operadora universal

						if($connec->query($sql)) {
							$sql = "UPDATE BDES.dbo.BISolicPedido WITH (READPAST) SET
										id_transportado = $id_act
			...
			case 'procesarSoliPedi':
				...
				if(count($insert)>0) {
					$sql = "INSERT INTO BDES.dbo.BISoliPedTran(nro_despacho, nro_transferencia, nro_pedido) VALUES";
				...
				if($sql) {
					...
					$result = '0¬Hubo un error, no se procesaron las ordenes' . chr(10) . $sql2;
				}
			...
			case 'exportVtasBiplus2TNS':
				...
				$sql = "select localidad, 403471 as codigo,tipo,porc,sum(base) as base,sum(impuesto) as impuesto from (
						...
						select localidad, 403470 as codigo,tipo,porc,sum(base) as base,sum(impuesto) as impuesto from (
						SELECT localidad, subgrupo, TIPO, cast(PORC as numeric) as porc,SUM(SUBTOTAL) AS BASE, SUM(IMPUESTO) AS impuesto 
						FROM (SELECT v.localidad, coalesce(a.subgrupo,0) as Subgrupo,a.codigo,
						v.Tipo - 26 as Tipo,CASE WHEN v.IMPUESTO = 0 THEN 0 ELSE (v.IMPUESTO * 100 )/ v.SUBTOTAL END AS PORC,v.Subtotal,v.Impuesto
						From bdes_pos.dbo.BIVentas_Inv v left join bdes.dbo.ESArticulos a on 
						a.codigo=v.material WHERE CAST(v.FECHA AS DATE) = '$fecha' and $localidad and  v.material ='1403470') AS tb
						GROUP BY TIPO, cast(PORC as numeric), subgrupo, tb.localidad ) as j 
						where  porc ='0' 
						group by tipo,porc,localidad
						...
						select localidad, 100000 as codigo,tipo,porc,sum(base) as base,sum(impuesto) as impuesto from (
						SELECT localidad, subgrupo, TIPO, cast(PORC as numeric) as porc,SUM(SUBTOTAL) AS BASE, SUM(IMPUESTO) AS impuesto 
						FROM (SELECT v.localidad, coalesce(a.subgrupo,0) as Subgrupo,a.codigo,
						v.Tipo - 26 as Tipo,CASE WHEN v.IMPUESTO = 0 THEN 0 ELSE (v.IMPUESTO * 100 )/ v.SUBTOTAL END AS PORC,v.Subtotal,v.Impuesto
						From bdes_pos.dbo.BIVentas_Inv v left join bdes.dbo.ESArticulos a on 
						a.codigo=v.material WHERE CAST(v.FECHA AS DATE) = '$fecha' and $localidad and  v.material not in ('1403470')) AS tb
						GROUP BY  TIPO, cast(PORC as numeric), subgrupo, tb.localidad ) as j
						where  porc ='0'
						group by tipo,porc,localidad
				...
				echo json_encode(array('enlace'=>'tmp/TNS_VE'.$icorr.$ext.'.xls', 'archivo'=>'TNS_VE'.$icorr.$ext.'.xls'));
				break;
			...
			case 'consDispPerecederos':
				...
				$sql = "SELECT
						...
						LEFT JOIN	(SELECT articulo AS Material,
								...
								(CASE WHEN localidad = 6 AND almacen = 601 THEN
				...
				if(!$sql) {
					...
					if(!$sql) {
						...
						if(!$sql) {
							...
							$thtabla.= '<tr>';
							$thtabla.= '<th id="th0" class="bg-primary-gradient">CÓDIGO</th>';
							$thtabla.= '<th id="th1" class="bg-primary-gradient">DESCRIPCIÓN</th>';
							$thtabla.= '<th id="th2" title="Cantidad para la Compra" class="bg-warning-gradient">PEDIR</th>';
							$thtabla.= '<th id="th3" title="Total General UND." class="bg-success-gradient">TOTAL</th>';
							$i=4;
							foreach ($tiendas as $tienda) {
								$thtabla.= '<th id="th'.$i.'" title="'.$tienda['nombre'].'"';
								$thtabla.= ' class="bg-secondary-gradient">' . substr($tienda['nombre'], 0, 5) . '</th>';
								$i++;
							}
							...
							foreach ($articulos as $articulo) {
								$tabla.= '<tr>';
								$tabla.= '<td width="6%">' . $articulo['codigo'] . '</td>';
								$tabla.= '<td width="22%">' . $articulo['descripcion'] . '</td>';
							...
							$fotabla.= '<th id="fo2" data-val="0" class="bg-warning-gradient"></th>';
							$fotabla.= '<th id="fo3" data-val="0" class="bg-success-gradient"></th>';
							$i=4;
							foreach ($tiendas as $tienda) {
			...
			case 'procPediPerecederos':
				...
				if($sql){
					...
					$sql = "INSERT INTO BDES.dbo.DBCmpPerecederosD(id_cab, codigo, cantidad)
							VALUES ";
					for ($i=0; $i < count($codigos); $i++) {
						$sql .= "(" . $codid . "," . $codigos[$i] .", ".$cantidad[$i]."),";
					}
					$result['query'] = $sql;
					$sql = $connec->query(substr($sql, 0, -1));
					...
					if($sql){
						...
						$result['mensaje'] = 'Hubo un error, no se creo el detalle de la orden<br>' . $connec->errorInfo()[2];
					}
				} else {
			...
			case 'artDispoPerecederos':
				...
				while ($row = $sql->fetch(\PDO::FETCH_ASSOC)) {
					$datos[] = [
						...
						'barra'       => $row['barra'],
						'descripcion' => '<button type="button" title="Seleccionar" onclick="' .
											"agregararticulo(".$row['codigo'].", '".$row['descripcion']."')".
											'" class="btn btn-sm btn-link m-0 p-0 text-left font-weight-bold" ' .
											'style="white-space: normal; line-height: 1;">' .
											$row['descripcion'].
										'</button>',
			...
			case 'genHDTPerecederos':
				...
				$sql = "SELECT DISTINCT det.codigo, art.descripcion
				...
				if($result==1) {
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, 'COMPRAR');
					$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, 'PEDIDOS');
					...
					foreach ($articulos as $articulo) {
						...
						$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $cantcompra);
						
						$key = array_search($articulo['codigo'], array_column($totales, 'codigo'));
						if($key!==false) {
							// $objPHPExcel->getActiveSheet()
							// 	->SetCellValue('D'.$rowCount, $totales[$key]['pedido']);
							
							$objPHPExcel->getActiveSheet()
								->SetCellValue('D'.($rowCount),
									'=SUM(E'.$rowCount.':'.chr(68+count($tiendas)).$rowCount.')');
			...
			case 'addArtTMPCmp':
				$sql = "SELECT 0 AS id, art.codigo, art.descripcion, 0 AS cantidad,	0 AS pedido,
							COALESCE(uc.costo, 0) AS costo, COALESCE(uc.precio1, 0) AS precio1,
							art.impuesto
						FROM BDES.dbo.ESARTICULOS AS art
						LEFT JOIN BDES.dbo.BIDocumentoSincroUCosto AS uc ON uc.articulo = art.codigo
							AND uc.sucursal = 6
				...
				while ($row = $sql->fetch(\PDO::FETCH_ASSOC)) {
					...
					$datos[] = [
						...
						'boton'       => '<button id="btn'.$row['codigo'].'" class="btn btn-info elevation-2 '.
			...
			case 'saveTMPCmp':
				...
				$sql = "MERGE INTO BDES.dbo.DBGesCompra_TMP AS destino
						USING (SELECT 
							...
							'".$datos['pvpver']."' AS pvpver) AS origen
						ON proveedor = $idprov AND usuario = '$userid' AND centro_dist = 6 AND codigo = ".$datos['codigo']."
						WHEN MATCHED THEN 
							UPDATE SET
						...
						WHEN NOT MATCHED THEN
							INSERT(proveedor, usuario, centro_dist, codigo, descripcion, desctxt, impuesto,
									boton, cntnva, cntori, cntver, cstnva, cstori, cstver, marnva, marori,
									marver, prenva, preori, prever,	pvpnva, pvpori, pvpver)
							VALUES($idprov, '$userid', 6, ".$datos['codigo'].", origen.descripcion,
									...
									origen.pvpnva, origen.pvpori, origen.pvpver);";
			...
			case 'proGESCmp':
				...
				foreach ($idpara as $datos) {
					if($datos['codigo']!=0) {
						...
						$sql = "MERGE INTO BDES.dbo.DBKardexCompras AS destino ".
								...
								"status   = (CASE WHEN (destino.comprado+origen.comprado)>=destino.cantidad ".
											"THEN 1 ELSE 0 END) ".
								"WHEN NOT MATCHED THEN ".
								"INSERT(codigo, cantidad, comprado, status)".
								"VALUES($codigo, origen.comprado, origen.comprado, 1);";
						$result['query'] = substr($sql, 0, 40);
						$sql = $connec->query($sql);
						if(!$sql || $sql->rowCount()==0) {
							$result['query'] = '<em>'.$result['query'].'<br>'.
				...
				if(!$sql || $sql->rowCount()==0) {
					...
					$result['query'] = $sql; //substr($sql, 0, 40);
					$sql = $connec->query(substr($sql, 0, -1));
					if(!$sql || $sql->rowCount()==0) {
						$result['query'] = '<em>'.$result['query'].'<br>'.
							substr($connec->errorInfo()[2], strrpos($connec->errorInfo()[2], ']')+1).
			...
			case 'lstArtxProv':
				extract($_POST);
				$sql = "SELECT * FROM BDES.dbo.DBGesCompra_TMP
						WHERE proveedor = $idpara AND centro_dist = 6
						AND usuario = '$userid'";
				...
				$sql = "SELECT ped.id, ped.codigo, art.descripcion, ped.cantidad AS cantidad,
							(CASE WHEN ped.comprado > 0 THEN
								(ped.cantidad-ped.comprado-ped.nocomprar) ELSE ped.cantidad END) AS pedido,
							COALESCE(uc.costo, 0) AS costo, COALESCE(uc.precio1, 0) AS precio1,
							art.impuesto
						FROM BDES.dbo.DBKardexCompras AS ped
						INNER JOIN BDES.dbo.ESARTICULOS AS art ON art.codigo = ped.codigo
						LEFT JOIN BDES.dbo.DBProvArticulos AS pro ON pro.articulo = ped.codigo
							AND pro.eliminado = 0
						LEFT JOIN BDES.dbo.BIDocumentoSincroUCosto AS uc ON uc.articulo = ped.codigo
							AND uc.sucursal = 6
						WHERE pro.proveedor = $idpara
						AND ped.status = 0
						AND ABS(ped.cantidad-ped.comprado-ped.nocomprar) > 0
						ORDER BY art.descripcion";
			...
			case 'lstHDTant':
				extract($_POST);
				$sql = "SELECT DISTINCT id, fecha_registro AS fecha
						FROM BDES.dbo.DBCmpPerecederosC
			...
			case 'detHDTant':
				...
				$sql = "SELECT det.codigo, art.descripcion, det.cantidad
			...
			case 'lstPenXBuyPerece':
				$sql = "SELECT a.codigo, a.descripcion,
							( SELECT TOP 1 barra FROM BDES.dbo.ESCodigos AS b
							WHERE b.escodigo = a.codigo AND b.codigoedi = 1) AS barra,
							(k.cantidad-k.comprado-k.nocomprar) AS pendiente
						FROM BDES.dbo.DBKardexCompras k
						INNER JOIN BDES.dbo.ESARTICULOS a ON a.codigo = k.codigo
						WHERE a.activo = 1 AND (k.cantidad-k.comprado-k.nocomprar) > 0";
			...
			case 'cerrarCompra':
				$idpara = explode('¬', $idpara);
				$nocomprar = ($idpara[0]=='*') ? 'cantidad-comprado' : $idpara[1];
			...
			case 'busDatosBono':
				extract($_POST);
				$idpara = explode('¬', $idpara);
				$fechai = $idpara[0] . ':00.000';
				$fechaf = $idpara[0] . ':59.999';
				...
				$sql = "SELECT id_beneficiario, monto, referencia, cli.RAZON
						...
						WHERE bonos.fecha BETWEEN '$fechai' AND '$fechaf'
						AND bonos.id_beneficiario = '$idpara[1]'
						AND bonos.id_empresa = '$idclte'";
